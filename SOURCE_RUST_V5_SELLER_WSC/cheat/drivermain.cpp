...
void maindriv::initialize()
{
	WSADATA wsa_data;
	WSAStartup(MAKEWORD(2, 2), &wsa_data);
}

void maindriv::deinitialize()
{
	WSACleanup();
}

SOCKET maindriv::connect()
{
	SOCKADDR_IN address{ };
...
void maindriv::disconnect(const SOCKET connection)
{
	closesocket(connection);
}

uint32_t maindriv::read_memory(
	const SOCKET	connection,
...
uint32_t maindriv::write_memory(
	const SOCKET	connection,
	const uint32_t	process_id,
	const uintptr_t address,
	const uintptr_t buffer,
...
uint64_t maindriv::get_process_base_address(const SOCKET connection, const uint32_t process_id, int module)
{
	Packet packet{ };
	JU();
	packet.header.magic = packet_magic;
